...
namespace Zorro.Core.CLI
{
	...
	public class ConsoleCommands
	{
		...
		public static List<ConsoleCommand> ConsoleCommandMethods = new List<ConsoleCommand>
		{
			...
			new ConsoleCommand(new Action(AchievementManager.UnlockAll).Method),
			...
			new ConsoleCommand(new Action(Character.RagdollExorcism).Method),
			...
			new ConsoleCommand(new Action<bool>(CharacterRagdoll.SetKinematic).Method),
			new ConsoleCommand(new Action<float>(CharacterStats.DebugTickRate).Method),
			new ConsoleCommand(new Action(CharacterSyncer.SnapToRemoteValues).Method),
			...
			new ConsoleCommand(new Action<RunSettings.SETTINGTYPE, bool>(RunSettings.Increment).Method),
			new ConsoleCommand(new Action<RunSettings.SETTINGTYPE>(RunSettings.PrintValue).Method),
			new ConsoleCommand(new Action(RunSettings.ResetAll).Method),
			new ConsoleCommand(new Action(RunSettings.Test).Method),
		...
		public static Dictionary<Type, CLITypeParser> TypeParsers = new Dictionary<Type, CLITypeParser>
		{
			...
			{
				typeof(RunSettings.SETTINGTYPE),
				new RunSettingCLIParser()
			},
			{
				typeof(bool),
				new BoolCLIParser()
